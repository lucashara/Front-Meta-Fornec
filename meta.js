...
async function uploadFile() {
  ...
  try {
    ...
    const response = await fetch(
      "https://api.grupobrf1.com:4443/atualizar_meta_fornec_rca/",
      {
        method: "POST",
        // Removido o header 'token' já que não estamos mais usando JWT
        body: formData,
      }
    );
    ...
    if (response.ok) {
      messageElement.textContent =
        "Arquivo enviado com sucesso. Resposta da API: " +
        JSON.stringify(responseData);
    } else {
      messageElement.textContent =
        "Falha no upload do arquivo. Resposta da API: " +
        JSON.stringify(responseData);
    }
  } catch (error) {
